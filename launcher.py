...
def prepare_environment():
    ...
    run_pip(f"install -r \"{requirements_file}\"", "requirements for SynchroFace WebUI (may take longer time in first time)")
# this scripts installs necessary requirements and launches main program in webui.py
# borrow from : https://github.com/AUTOMATIC1111/stable-diffusion-webui/blob/master/launch.py
import subprocess
import os
...
def prepare_environment():
    ...
    if sys.platform != 'win32' and not is_installed('tts'):
        run_pip(f"install TTS", "install TTS individually in SynchroFace, which might not work on windows.")


def start():
    print(f"Launching SynchroFace Web UI")
    from app_synchroface import synchroface_demo
